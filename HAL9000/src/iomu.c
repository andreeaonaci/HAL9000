    ...
    )
{
    ...
    else
    {
        ...
        DWORD bitmapSize = BitmapPreinit(&m_iomuData.SwapBitmap, ((DWORD)m_iomuData.SwapFileSize / PAGE_SIZE)+1);
...
)
{
    ...
    QWORD swapOffset;
    INTR_STATE dummyState;
    INTR_STATE dummyState1;
    PFILE_OBJECT pSwapFile;
    QWORD writtenBytes;
    STATUS status;
    PPROCESS pCurrProcess;
    PVATOSWAP pVatToSwap;

    ASSERT(NULL != VirtualAddress);

    #pragma warning(suppress:4047)
    AlignedAddress =AlignAddressLower(VirtualAddress, PAGE_SIZE);

    LockAcquire(&m_iomuData.SwapBitmapLock, &dummyState);
    index = BitmapScanAndFlip(&m_iomuData.SwapBitmap, 1, FALSE);
    LockRelease(&m_iomuData.SwapBitmapLock, dummyState);

    if (MAX_DWORD == index)
	{
		LOG_ERROR("No more space in swap file\n");
		return STATUS_UNSUCCESSFUL;
	}

    swapOffset = index * PAGE_SIZE;

    pSwapFile = IomuGetSwapFile();
    ASSERT(NULL != pSwapFile);

    status = IoWriteFile(pSwapFile, PAGE_SIZE, &swapOffset, (PVOID)AlignedAddress, &writtenBytes);

    if (!SUCCEEDED(status) || writtenBytes != PAGE_SIZE)
	{
		LOG_FUNC_ERROR("IoWriteFile", status);
        BitmapSetBitValue(&m_iomuData.SwapBitmap, index, FALSE);
		return status;
	}

    pCurrProcess = GetCurrentProcess();

    pVatToSwap = ExAllocatePoolWithTag(PoolAllocateZeroMemory, sizeof(VATOSWAP), HEAP_IOMU_TAG, 0);
    ASSERT(NULL != pVatToSwap);

    pVatToSwap->SwapAddress = swapOffset;
    pVatToSwap->VirtualAddress = AlignedAddress;

    LockAcquire(&pCurrProcess->MappingListLock, &dummyState1);
    InsertHeadList(&pCurrProcess->MappingList, &pVatToSwap->MappingListMember);
    LockRelease(&pCurrProcess->MappingListLock, dummyState1);

    MmuUnmapMemoryEx(AlignedAddress, PAGE_SIZE, FALSE, pCurrProcess->PagingData);
