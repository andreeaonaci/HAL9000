    ...
    )
{
    ...
    ASSERT(NumberOfParameters == 0);
    LOG("\n");
    LOG("All threads = %d", ThreadGetAllThreadsList());
    LOG("%7s", "TID|");
    LOG("%20s", "Name|");
    LOG("%5s", "Prio|");
    LOG("%8s", "State|");
    ...
    )
{
    ...
    {
        PTHREAD pChildThread = CONTAINING_RECORD(pChild, THREAD, ChildrenList);
        LOG("%9x%c", pChildThread->Id, '|');

        pChild = pChild->Flink; // Move to the next child
    }
    ...
    )
{
    ...
    PVOID memory;
    memory = ExAllocatePoolWithTag(PoolAllocateZeroMemory, PAGE_SIZE, HEAP_TEST_TAG, 0);
    memset(memory, 0x40, PAGE_SIZE);
